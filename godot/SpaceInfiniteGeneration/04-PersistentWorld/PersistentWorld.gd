...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed:
		...
		if click_position.x < player.global_position.x:
			sector_key.x -= 1
		if click_position.y < player.global_position.y:
			sector_key.y -= 1
	...
	for key in sectors.keys():
		...
		if sectors[key].size() > 3:
			for path in sectors[key][3]:
				...
				draw_line(start, end, Color.cornflower, 12.0)
...
func _generate_planets_at(x: int, y: int) -> void:
	...
	var should_remove_planet: bool = modifications.has(key) and modifications[key].has("remove_generated_planet") and modifications[key].remove_generated_planet
	var should_force_planet: bool = modifications.has(key) and modifications[key].has("force_planet") and modifications[key].force_planet
