...
export var planet_formation_threshold := 10000.0
...
func _planet_generate_at(_x_id: int, _y_id: int) -> void:
	...
	var sector_position := Vector2(_x_id, _y_id) * sector_size
	...
	for x in range(-1, 2):
		for y in range(-1, 2):
			...
			var key := Vector2(_x_id + x, _y_id + y)
			...
			for asteroid in sector_data:
				...
				if asteroid.is_in_group("planets"):
					return
	
	if asteroid_weight > planet_formation_threshold:
		var planet := Planet.instance()
		add_child(planet)
		planet.global_position = sector_position
		sectors[Vector2(_x_id, _y_id)].append(planet)
		planet.setup(rng)
