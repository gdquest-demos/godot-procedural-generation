...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.button_index == BUTTON_LEFT:
		_map[grid_position] = PlantState.DIED if not _map[grid_position] == PlantState.DIED else PlantState.GROWN
		_paint_map()
...
func _initialize_map() -> void:
	...
	for x in range(_grid_size.x):
		for y in range(_grid_size.y):
			_map[Vector2(x, y)] = PlantState.GROWN if randf() < chance_to_start_alive else PlantState.DIED
...
func _count_alive_neighbors(map: Dictionary, location: Vector2) -> int:
	...
	for neighbor in _neighbors_moore:
		...
		if check_location.x < 0 or check_location.x >= _grid_size.x or check_location.y < 0 or check_location.y >= _grid_size.y:
			continue
