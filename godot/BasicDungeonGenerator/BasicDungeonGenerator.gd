...
func _generate_data() -> Array:
	...
	for r in range(rooms_max):
		...
		_add_room(data, rooms, room, rng)
		if rooms.size() > 1:
			var room_previous: Rect2 = rooms[-2]
			_add_connection(data, room_previous, room, rng)
	return data.keys()
...
func _add_room(data: Dictionary, rooms: Array, room: Rect2, rng: RandomNumberGenerator) -> void:
...
func _add_connection(
	data: Dictionary, room1: Rect2, room2: Rect2, rng: RandomNumberGenerator
) -> void:
